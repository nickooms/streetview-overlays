...
function toGradientMagnitude(xDerived, yDerived, width, height, lt = 0, ut = 0) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      index = y * width + x;
      gradientMagnitude[index] = Math.sqrt(
        xDerived[index] * xDerived[index] + yDerived[index] * yDerived[index]
      );
      pom = Math.atan2(xDerived[index], yDerived[index]);
      if (
        (pom >= -Math.PI / 8 && pom < Math.PI / 8) ||
        (pom <= (-7 * Math.PI) / 8 && pom > (7 * Math.PI) / 8)
      ) {
        gradientDirection[index] = 0;
      } else if (
        (pom >= Math.PI / 8 && pom < (3 * Math.PI) / 8) ||
        (pom <= (-5 * Math.PI) / 8 && pom > (-7 * Math.PI) / 8)
      ) {
        gradientDirection[index] = Math.PI / 4;
      } else if (
        (pom >= (3 * Math.PI) / 8 && pom <= (5 * Math.PI) / 8) ||
        ((-3 * Math.PI) / 8 >= pom && pom > (-5 * Math.PI) / 8)
      ) {
  ...
  for (var y = 1; y < height - 1; y++) {
    for (var x = 1; x < width - 1; x++) {
      ...
      if (
        gradientDirection[index] == 0 &&
        (gradientMagnitudeLt[index] <= gradientMagnitudeLt[y * width + x - 1] ||
          gradientMagnitudeLt[index] <= gradientMagnitudeLt[y * width + x + 1])
      )
        gradientMagnitudeLt[index] = 0;
      else if (
        gradientDirection[index] == Math.PI / 2 &&
        (gradientMagnitudeLt[index] <= gradientMagnitudeLt[(y - 1) * width + x] ||
          gradientMagnitudeLt[(y + 1) * width + x] >= gradientMagnitudeLt[index])
      )
        gradientMagnitudeLt[index] = 0;
      else if (
        gradientDirection[index] == Math.PI / 4 &&
        (gradientMagnitudeLt[index] <= gradientMagnitudeLt[(y + 1) * width + x - 1] ||
          gradientMagnitudeLt[index] <= gradientMagnitudeLt[(y - 1) * width + x + 1])
      )
        gradientMagnitudeLt[index] = 0;
      else if (
