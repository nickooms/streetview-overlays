...
$file.addEventListener('change', (event) => {
  ...
  readFileAsDataURL(file)
    .then(loadImage)
    .then(setCanvasSizeFromImage(document.querySelector('#panorama-canvas') /* canvasFrom */))
    .then(drawImageOnCanvas(document.querySelector('#panorama-canvas') /* canvasFrom */))
    ...
    .then(() => {
      [...document.querySelectorAll('canvas')].forEach((canvas) => {
        /* const newWidth = Math.min($result.clientWidth, canvas.width);
...
function resetImageNav() {
  [...document.querySelectorAll('.image-nav__item--active')].forEach((el) =>
    el.classList.remove('image-nav__item--active')
  );
...
function onWorkerMessage(e) {
  ...
  if (type === 'grayscale') {
    ...
    drawBytesOnCanvasForImg(canvasGrayscale, data);
...
document.querySelector('.js_image-nav').addEventListener('click', (e) => {
  ...
  if (eventTarget.classList.contains('image-nav__item--active')) {
    ...
    Array.from(document.querySelectorAll('.image--active')).forEach((el) =>
